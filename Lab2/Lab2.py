...
def part4():
    ...
    #     if s == 3: 
    #         receive += 1
    #         bit1 = nSidedDie([E1, 1 - E1]) - 1
    #         bit2 = nSidedDie([E1, 1 - E1]) - 1
    #         bit3 = nSidedDie([E1, 1 - E1]) - 1
    #         totalBit = bit1 + bit2 + bit3
    #         if totalBit < 2: # if theres more 1s in R then D = 1
    #             fail += 1
                
    #     if s == 0: 
    #         receive += 1
    #         bit1 = nSidedDie([1 - E0, E0]) - 1
    #         bit2 = nSidedDie([1 - E0, E0]) - 1
    #         bit3 = nSidedDie([1 - E0, E0]) - 1
    #         totalBitty = bit1 + bit2 + bit3
    #         if totalBit > 1: # if theres more 0s in R then D = 0
    #             fail += 1
    
    # #r.append(totalBitty) #add it to our array
    # #majority = sum(r) #use sum to add up all the 0 and 1s
